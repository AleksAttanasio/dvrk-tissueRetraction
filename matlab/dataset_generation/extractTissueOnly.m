function [  ] = extractTissueOnly( class, dest_dir)
% The function extract the train images and relative labels of the dataset
% defined by the 'class' parameter. Images are selected from the ones only
% containing tissue without any overlapped tool.
...
for k = 1: size(labels, 1)
    ...
    for i = 1 : size(labels(k).gTruth.LabelData, 1)
        ...
        if ~isempty(labels(k).gTruth.LabelData.tissue{i,1}) && ...
            ...
            % save file in folder
            path = fullfile(dest_dir,strcat(class, '_dataset_tissue_only'));
